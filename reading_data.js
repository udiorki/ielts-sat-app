...
function generateStandardTest(bookNum, testNum) {
  const tIdx = ((bookNum - 1) * 3) % topicsPool.length;
  const top1 = topicsPool[tIdx % topicsPool.length];
  const top2 = topicsPool[(tIdx + 1) % topicsPool.length];
  const top3 = topicsPool[(tIdx + 2) % topicsPool.length];
  
  return {
    bookId: String(bookNum),
    testId: String(testNum),
    passages: [
      {
        id: 1,
        title: top1[0],
      ...
      },
      {
        ...
        questions: [
          ...
          { id: 38, type: "tfng", prompt: "All computational labs agree on the exact mathematical formulas for cascading transformations.", options: ["YES", "NO", "NOT GIVEN"], answer: "NOT GIVEN" formulas for cascading transformations.", options: ["YES", "NO", "NOT GIVEN"], answer: "NOT GIVEN" },
...
// Заполняем все 21 книгу
for (let b = 1; b <= 21; b++) {
  const bStr = String(b);
  window.CAMBRIDGE_DATA[bStr] = window.CAMBRIDGE_DATA[bStr] || {};
  for (let t = 1; t <= 4; t++) {
    window.CAMBRIDGE_DATA[bStr][String(t)] = generateStandardTest(b, t);
